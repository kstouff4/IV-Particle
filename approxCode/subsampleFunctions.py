...
def reverseOneHot(nu_Z,indsToKeep,maxVal=673):
 ...
 def makeSubsampleFun(X, nu_X, C=1.2,dis=True, maxV=673):
    # Input:
...
def makeSubsample(Xfile,sigma,savename,xtype='discrete',ztype='semi-discrete',overhead=0.1,maxV=673,C=1.2):
    '''
    Types of sampling:
    ...
    nu_X = xinfo['nu_X']
    if (xtype == 'discrete'):
        Z, nu_Z = makeSubsampleFun(X,nu_X,C=C,dis=True,maxV=maxV)
        # write original Z 
        ...
        Z, nu_Z = makeSubsampleFun(X,nu_X,C=C,dis=False,maxV=maxV)
        # write original Z 
        maxInd = np.argmax(nu_Z,axis=-1)+1
