...
for f in ${fils[*]}; do
    ...
    pref3=${pref2%.npz}
    outPref="${outPathZ}"
    python3 -c "fs='$f'; pref=fs.split('/')[-1]; p=pref[1:4]; print(p); r=int(p); p = pref.replace('.npz',''); i=p[-1]; z = (r-201)*100;import subsampleFunctions as sf; sf.makeSubsampleStratified('$f','X','nu_X', $sigma, '${outPathZ}_' + str(r) + '_' + str(i), xtype='discrete',ztype='uniform',overhead=$delta,maxV=702,C=1.0,dim=2,z=z); quit()"    
done
